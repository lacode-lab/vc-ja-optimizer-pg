...
最後に「このまま指示を出しますか？」と確認されるので、「はい」と返せばその内容がそのまま実行されます。実行せず他の場所（Issue、チャットなど）に使いたい場合は、クリップボードにコピー済みのテキストを貼り付けてください。

> **Note**: 確認はAskUserQuestion（選択ダイアログ）ではなく、テキストで尋ねてターンを終了する方式にしています。このスキルは `model: sonnet` + `effort: low` で動くため、同一ターン内で実行まで行うと本題までSonnetで実行されてしまいます。「はい」の返事は新しいターンになるため、本題はセッションのメインモデル（Opusなど）で実行されます。
